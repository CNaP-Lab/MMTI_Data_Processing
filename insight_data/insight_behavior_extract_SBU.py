...
"""
	Developed By: Sameera Abeykoon 
        ...
        This will extarct Metcog_insight pilot project Insight
        Behavior data
        Note : original txt files were ritten in *.edt2 format
               so needed to copy it to => txt file
               (Needed to open Edit: select all and copy into 
                copy paste to new file so created 50081-2.txt files
...
num_list = []
t_file = raw_input("Please enter the .txt file ? " )
# txtfile = "/mnt/hcp01/insight_task/Insight_task/InsightExample/13015-2.txt"
# dir_path = os.path.dirname(txtfile)
dir_path = "/mnt/hcp01/insight_task/Insight_task/Insight_behavior_txt/"
txtfile = dir_path + t_file
...
with open(dei_file, 'w') as file:
    ...
    # calculate the SOA values and Onset values
    j = 0
    Onset_val = []
    SOA_val = []
    for k in add_level:
    ...
    for j, value in enumerate(rawStim):
        ...
        # get the response and NAN's for each category
        try:
            new_resp = int(score)
            new_RT = int(RT[j])
    ...
    file.write("# Waiting for MR sync... \n")
    file.write("## --------------------------------------------------------------------------------------------------- \n")
    file.write("## For trials that had responses ('resps'), r(esponses)Mean, rtTot(al), rtMean \n")
    file.write("## category NaNs resps rMean rtTot rtMean \n")
    print (ds_resp, ds_RT, do_resp, do_RT, fs_resp, fs_RT, fo_resp, fs_RT)
