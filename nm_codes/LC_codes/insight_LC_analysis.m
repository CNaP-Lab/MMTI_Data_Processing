clear all
close all
root_folder = ['/mnt/hcp01/test_folder']
...
for s=1:length(LC_key(:,1))
    ...
    elseif LC_key(s,2)==3
        ...
        NMimage = spm_read_vols(v);
    elseif LC_key(s,2)==4
        LCmask = spm_read_vols(spm_vol([root_folder '/LC_nm_data/LCMNO' Subjects{s} '.nii']));
        v=spm_vol([root_folder '/LC_nm_data/NM' Subjects{s} '.nii']);
        NMimage = spm_read_vols(v);
    end
...
%     end
    ...
    spm_write_vol(v,ref_reg_mask3D);
    elseif LC_key(s,2)==4
    v.fname = [root_folder '/LC_nm_data/ref_LC' Subjects{s} '.nii'];
    spm_write_vol(v,ref_reg_mask3D);
    end
