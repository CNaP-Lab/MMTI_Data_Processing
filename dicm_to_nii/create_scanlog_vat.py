...
for i in np.sort(os.listdir(path)):
    ...
    if os.path.isdir(path+"/"+i):
            ...
            if 'SpinEchoFieldMap' in filename and "PA" in filename and 'Task' not in filename and 'SBRef' not in filename:
            ...
            elif 'SpinEchoFieldMap' in filename and "AP" in filename and 'Task' not in filename and 'SBRef' not in filename:
...
for i in range(len(mapoutlist)):
	...
	if 'Topup_SBRef_AP' in  q[j]:
        ...
        if 'B0' in  q[j]:
            ...
            allstring[allstudy.index(str(mapoutlist[i]))].append(' ' + str(s[j]) + ' 12:00 ' +  str(r[j]) + ' ' + q[j] + ' ' + str(previousb0echo) +  ' ' + str(0) + ' ' + str(0))
