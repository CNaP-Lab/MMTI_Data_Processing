...
topup_path = "/mnt/jxvs01/incoming/NYSPI_data/physical_disk_K01/" + s_number + "/Topup"
...
ap_f = open(ap_pa_file, "w")
ap_f.write("# " + scan_file)
ap_f.write('\n')
...
for i in (arr):
    ...
    if "Topup_PA" in i[4]:
        pa_dir = topup_path + "/" + i[1]
        print (pa_dir)
        pa_file = hcp_field + "/" + s_number + "_3T_SpinEchoFieldMap_PA_" + str(m) + ".nii.gz"
        dicom2nifti.dicom_series_to_nifti(pa_dir,pa_file, reorient_nifti=True)
        m = m + 1

    if "Topup_AP" in i[4]:
        ap_dir = topup_path + "/" + i[1]
        ap_file = hcp_field + "/" + s_number + "_3T_SpinEchoFieldMap_AP_" + str(n) + ".nii.gz"
        dicom2nifti.dicom_series_to_nifti(ap_dir,ap_file, reorient_nifti=True)
        n = n + 1
    ...
    if "T1w" in i[4]:
        ...
        if os.path.lexists(src):
             os.remove(src)
        os.symlink(dst, src)
        # create symbolic link for SpinEchoFielMap PA
        dst_pa = hcp_field + "/" + s_number + "_3T_SpinEchoFieldMap_PA_" + i[6] + ".nii.gz"
        src_pa = t1mpr + "/" + s_number + "_3T_SpinEchoFieldMap_PA.nii.gz"
        if os.path.lexists(src_pa):
            os.remove(src_pa)
        os.symlink(dst_pa, src_pa)
        # create symbolic link for SpinEchoFielMap AP
        dst_ap = hcp_field + "/" + s_number + "_3T_SpinEchoFieldMap_AP_" + i[6] + ".nii.gz"
        src_ap = t1mpr + "/" + s_number + "_3T_SpinEchoFieldMap_AP.nii.gz"
        if os.path.lexists(src_ap):
            os.remove(src_ap)
        os.symlink(dst_ap, src_ap)
    ...
    if "T2w" in i[4]:
        ...
        # create symbolic link for SpinEchoFielMap PA
        dst_pa = hcp_field + "/" + s_number + "_3T_SpinEchoFieldMap_PA_" + i[6] + ".nii.gz"
        src_pa = t2mpr + "/" + s_number + "_3T_SpinEchoFieldMap_PA.nii.gz"
        if os.path.lexists(src_pa):
            os.remove(src_pa)
        os.symlink(dst_pa, src_pa)
        # create symbolic link for SpinEchoFielMap AP
        dst_ap = hcp_field + "/" + s_number + "_3T_SpinEchoFieldMap_AP_" + i[6] + ".nii.gz"
        src_ap = t2mpr + "/" + s_number + "_3T_SpinEchoFieldMap_AP.nii.gz"
        if os.path.lexists(src_ap):
            os.remove(src_ap)
        os.symlink(dst_ap, src_ap)
    ...
    if "fMRI" in i[4]:
          ...
          # create symbolic link for SpinEchoFielMap PA
          dst_pa = hcp_field + "/" + s_number + "_3T_SpinEchoFieldMap_PA_" + i[6] + ".nii.gz"
          src_pa = fmri_dir + "/" + s_number + "_3T_SpinEchoFieldMap_PA.nii.gz"
          if os.path.lexists(src_pa):
              os.remove(src_pa)
          os.symlink(dst_pa, src_pa)
          # create symbolic link for SpinEchoFielMap AP
          dst_ap = hcp_field + "/" + s_number + "_3T_SpinEchoFieldMap_AP_" + i[6] + ".nii.gz"
          src_ap = fmri_dir + "/" + s_number + "_3T_SpinEchoFieldMap_AP.nii.gz"
          if os.path.lexists(src_ap):
              os.remove(src_ap)
          os.symlink(dst_ap, src_ap)
